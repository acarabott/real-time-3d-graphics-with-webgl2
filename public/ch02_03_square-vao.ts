...
const initBuffers = (
    ...
    return { vertexBuffer, indexBuffer, vertexArrayObject };
};
...
const initProgram = async (gl: WebGL2RenderingContext) => {
    ...
    const vertexSource = await loadTextFile("./ch02_03_square-vao.vert");
    const vertexShader = loadShader(gl, "vertex", vertexSource);
    gl.attachShader(program, vertexShader);

    const fragmentSource = await loadTextFile("./ch02_03_square-vao.frag");
